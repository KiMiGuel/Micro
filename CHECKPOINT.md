...
1. Enumerate which Indepentest projects actually need an update-check feature. Confirmed so far: MicroVault, MeXiCOSINT. Do not assume the full list beyond those two — ask the user or discover it (check `~/CCM2`, `~/`, for other "Micro"/Indepentest repos) before implementing.
